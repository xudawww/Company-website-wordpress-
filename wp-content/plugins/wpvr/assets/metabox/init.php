...
spl_autoload_register('wpvr_cmb_Meta_Box::autoload_helpers');

$meta_boxes = array();
$meta_boxes = apply_filters( 'wpvr_cmb_meta_boxes', $meta_boxes );
// d( $meta_boxes );
foreach ( (array) $meta_boxes as $meta_box ) {
	$my_box = new wpvr_cmb_Meta_Box( $meta_box );
}

if( !defined('WPVR_CMB_META_BOX_URL') ) define( 'WPVR_CMB_META_BOX_URL', wpvr_cmb_Meta_Box::get_meta_box_url() );

/**
 * Create meta boxes
 */
class wpvr_cmb_Meta_Box {
	...
	function __construct( $meta_box ) {
		...
		$show_filters = 'wpvr_cmb_Meta_Box_Show_Filters';
	...
	public function register_scripts() {
		...
		$scripts = array( 'jquery', 'jquery-ui-core', 'wpvr_cmb-datepicker', /*'media-upload', */'wpvr_cmb-timepicker' );
		...
		if ( 3.5 <= $wp_version ) {
			...
			if ( ! is_admin() ) {
			   ...
			   wp_register_script( 'iris', admin_url( 'js/iris.min.js' ), array( 'jquery-ui-draggable', 'jquery-ui-slider', 'jquery-touch-punch' ), self::WPVR_CMB_VERSION );
		   	wp_register_script( 'wp-color-picker', admin_url( 'js/color-picker.min.js' ), array( 'iris' ), self::WPVR_CMB_VERSION );
				wp_localize_script( 'wp-color-picker', 'wpColorPickerL10n', array(
					'clear'         => __( 'Clear' ,  WPVR_LANG  ),
					'defaultString' => __( 'Default'  ,  WPVR_LANG ),
					'pick'          => __( 'Select Color'  ,  WPVR_LANG ),
					'current'       => __( 'Current Color' ,  WPVR_LANG  ),
				) );
			}
		} else {
		...
		wp_register_script( 'wpvr_cmb-datepicker', WPVR_CMB_META_BOX_URL . 'js/jquery.datePicker.min.js' );
		wp_register_script( 'wpvr_cmb-timepicker', WPVR_CMB_META_BOX_URL . 'js/jquery.timePicker.min.js' );
		wp_register_script( 'wpvr_cmb-scripts', WPVR_CMB_META_BOX_URL .'js/cmb'. $min .'.js', $scripts, self::WPVR_CMB_VERSION );

		wp_enqueue_media();

		wp_localize_script( 'wpvr_cmb-scripts', 'cmb_l10', apply_filters( 'cmb_localized_data', array(
			...
			'up_arrow'        => '[ ↑ ]&nbsp;',
			'down_arrow'      => '&nbsp;[ ↓ ]',
			'check_toggle'    => __( 'Select / Deselect All',  WPVR_LANG  ),
		) ) );

		wp_register_style( 'wpvr_cmb-styles', WPVR_CMB_META_BOX_URL . 'style'. $min .'.css', $styles );
	...
	public function add_metaboxes() {
		...
		if( isset($this->_meta_box['pages']) && is_array($this->_meta_box['pages']) ){
	...
	public static function show_form( $meta_box, $object_id = 0, $object_type = '' ) {
		...
		foreach ( (array) $meta_box['fields'] as $field_args ) {
			...
			if ( 'group' == $field_args['type'] ) {
				...
				$field = new wpvr_cmb_Meta_Box_field( $field_args );
				$field->render_field();
			}
		}
		echo '</table>';
	...
	public static function render_group( $args ) {
		...
		$args['count']   = 0;
		$field_group     = new wpvr_cmb_Meta_Box_field( $args );
		...
		if ( $desc || $label ) {
			echo '<tr><th>';
				if ( $label )
					echo '<h2 class="wpvr_cmb-group-name">'. $label .'</h2>';
				if ( $desc )
		...
		if ( ! empty( $group_val ) ) {
			...
			foreach ( (array) $group_val as $iterator => $field_id ) {
	...
	public static function render_group_row( $field_group, $remove_disabled ) {
		...
		<tr class="repeatable-grouping" data-iterator="'. $field_group->count() .'">
			<td>
				<table class="wpvr_cmb-nested-table" style="width: 100%;">';
				if ( $field_group->options( 'group_title' ) ) {
					echo '
					<tr class="wpvr_cmb-group-title">
						<th colspan="2">
				...
				foreach ( array_values( $field_group->args( 'fields' ) ) as $field_args ) {
					...
					$field = new wpvr_cmb_Meta_Box_field( $field_args, $field_group );
					$field->render_field();
				}
				echo '
					<tr>
	...
	public static function save_field( $new_value, $field ) {
		...
		// 	$field->remove_data();
		// 	if ( ! empty( $new_value ) ) {
		// 		foreach ( (array) $new_value as $add_new ) {
		// 			self::$updated[] = $name;
...
// Handle oembed Ajax
add_action( 'wp_ajax_cmb_oembed_handler', array( 'wpvr_cmb_Meta_Box_ajax', 'oembed_handler' ) );
add_action( 'wp_ajax_nopriv_cmb_oembed_handler', array( 'wpvr_cmb_Meta_Box_ajax', 'oembed_handler' ) );
 ...
 * @return array               Options array or specific field
 */
function wpvr_cmb_get_option( $option_key, $field_id = '' ) {
	return wpvr_cmb_Meta_Box::get_option( $option_key, $field_id );
}
 ...
 * @param  int    $object_id   Object ID
 * @param  string $object_type Type of object being saved. (e.g., post, user, or comment)
 * @return object              wpvr_cmb_Meta_Box_field object
 */
function wpvr_cmb_get_field( $field_args, $object_id = 0, $object_type = 'post' ) {
	// Default to the loop post ID
	$object_id = $object_id ? $object_id : get_the_ID();
	wpvr_cmb_Meta_Box::set_object_id( $object_id );
	wpvr_cmb_Meta_Box::set_object_type( $object_type );
	// Send back field object
	return new wpvr_cmb_Meta_Box_field( $field_args );
}
 ...
 */
function wpvr_cmb_get_field_value( $field_args, $object_id = 0, $object_type = 'post' ) {
 ...
 * @param int   $object_id  Object ID
 */
function wpvr_cmb_print_metaboxes( $meta_boxes, $object_id ) {
	foreach ( (array) $meta_boxes as $meta_box ) {
 ...
 * @param int   $object_id Object ID
 */
function wpvr_cmb_print_metabox( $meta_box, $object_id ) {
	$cmb = new wpvr_cmb_Meta_Box( $meta_box );
	if ( $cmb ) {

		wpvr_cmb_Meta_Box::set_object_id( $object_id );

		if ( ! wp_script_is( 'wpvr_cmb-scripts', 'registered' ) )
			$wpvr_cmb->register_scripts();
		...
		wpvr_cmb_Meta_Box::show_form( $meta_box );
	}
 ...
 * @param int   $object_id Object ID
 */
function wpvr_cmb_save_metabox_fields( $meta_box, $object_id ) {
	wpvr_cmb_Meta_Box::save_fields( $meta_box, $object_id );
}
 ...
 */
function wpvr_cmb_metabox_form( $meta_box, $object_id, $echo = true ) {

	$meta_box = wpvr_cmb_Meta_Box::set_mb_defaults( $meta_box );
	...
	wpvr_cmb_Meta_Box::set_object_type( wpvr_cmb_Meta_Box::set_mb_type( $meta_box ) );
	...
	if (
		...
		&& wp_verify_nonce( $_POST['wp_meta_box_nonce'], wpvr_cmb_Meta_Box::nonce() )
	...
	$form_format = apply_filters( 'cmb_frontend_form_format', '<form class="wpvr_cmb-form" method="post" id="%s" enctype="multipart/form-data" encoding="multipart/form-data"><input type="hidden" name="object_id" value="%s">%s<input type="submit" name="submit-cmb" value="%s" class="button-primary"></form>', $object_id, $meta_box, $form );

	$form = sprintf( $form_format, $meta_box['id'], $object_id, $form, __( 'Save' ,  WPVR_LANG  ) );
