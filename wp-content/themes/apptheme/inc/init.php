...
class AppPresser_Theme_Functions extends AppPresser_Theme_Setup {
	...
	public $Customizer;
	public $Tags;
	public $AppPresser_Mobile_Theme;
	public $AppPresser_Mobile_Admin_Settings;
	...
	protected function run_classes() {
		...
		} else if( file_exists( APPP_THEME_PATH . 'inc/classes/AppPresser_Mobile_Theme.php') ) {
			$this->AppPresser_Mobile_Theme = new AppPresser_Mobile_Theme();
			$this->run_hooks( 'AppPresser_Mobile_Theme' );

			$this->AppPresser_Mobile_Admin_Settings = new AppPresser_Mobile_Admin_Settings();
			$this->run_hooks( 'AppPresser_Mobile_Admin_Settings' );
		}
	...
	public function after_setup_theme() {
		...
		// acknowledge that they are not defining titles with the deprecated wp_title() WP 4.1
	...
	public function scripts_styles() {
		...
		if ( $use_concatenated ) {
			...
			// Help iOS close the keyboard when a click occurs anywhere but a text, password or textarea
			wp_enqueue_script( 'app-ios-keyboard-helper', APPP_THEME_URL ."js/app-ios-keyboard-helper$min.js", array('jquery'), '2.1', true );
		}

		$dependencies = $use_concatenated ? 'appp_plugins' : 'snap';

		wp_enqueue_script( 'appp-js', APPP_THEME_URL ."js/custom$min.js", array( $dependencies, 'app-ios-keyboard-helper' ), filemtime( get_template_directory() . "/js/custom$min.js" ), true );
		wp_enqueue_script( 'appp-modal', APPP_THEME_URL ."js/apppmodal$min.js", array('jquery'), '1.0', true );
		...
		if( class_exists( 'AppTheme_Pro' ) ) {
			$localized['can_ajax'] = AppTheme_Pro::get_can_ajax_logic();
		}

		wp_localize_script( 'appp-js', 'appp', $localized );
