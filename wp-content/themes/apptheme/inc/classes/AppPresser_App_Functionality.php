...
class AppPresser_App_Functionality {
				...
				<?php appp_login_modal_before();
					...
					if ( !is_user_logged_in() ) {
						if( class_exists( 'AppPresser' ) ) {
							echo '<p><a href="#app-lost-password" class="password-reset-btn io-modal-open">' . __('Lost Password?', 'apptheme') . '</a></p>';
						} else {
							// Without the Apppresser core plugin
							echo '<p><a href="'.wp_login_url().'">' . __('Lost Password?', 'apptheme') . '</a></p>';
						}
					}
				?>
			</div>
	...
	function comment_modal_template() {
		...
		<aside class="io-modal" id="commentModal" tabindex="-1" role="dialog" aria-hidden="true">
			...
			<div class="io-modal-content">
				...
				<h4><?php _e( 'Leave a comment', 'apptheme' ); ?></h4>

				<div id="comment-status" ></div>

				<p class="ajax-comment-form-author"><label for="author"><?php _e( 'Name', 'apptheme' ); ?> <span class="required">*</span></label> <input id="author" name="author" type="text" size="30" aria-required="true"></p>

				<p class="ajax-comment-form-email"><label for="email"><?php _e( 'Email', 'apptheme' ); ?> <span class="required">*</span></label> <input id="email" name="email" type="text" size="30" aria-describedby="email-notes" aria-required="true"></p>

				<p class="ajax-comment-form-url"><label for="url"><?php _e( 'Website', 'apptheme' ); ?></label> <input id="url" name="url" type="text" value="" size="30"></p>

				<p class="ajax-comment-form-comment"><label for="comment"><?php _e( 'Comment', 'apptheme' ); ?></label> <textarea id="comment" name="comment" cols="45" rows="8" aria-describedby="form-allowed-tags" aria-required="true"></textarea></p>

				<input type="hidden" id="ajax-comment-parent" value="0">

				<p id="ajax-comment-form-submit">
					<input name="submit" type="submit" id="submit" class="submit" value="<?php _e( 'Post Comment', 'apptheme' ); ?>">
				</p>
			</div>
		</aside>
		<?php
	...
	public function get_error_param() {
		...
		if ( isset( $_GET['errors'] ) && $_GET['errors'] == 'login_failed' )
			return __('Login Failed! Please try again.', 'apptheme');
	...
	function custom_login_styles() {
		...
		body.login div#login h1 a {
			background-image: url(<?php echo esc_url(get_template_directory_uri()) ?>/images/login-logo.png);
			padding-bottom: 40px;
			background-size: 90%;
		}
		body.login {
