...
class wpvr_cmb_Meta_Box_field {
	...
	public function __construct( $field_args, $group_field = null ) {
		$this->object_id   = wpvr_cmb_Meta_Box::get_object_id();
		$this->object_type = wpvr_cmb_Meta_Box::get_object_type();
	...
	public function get_data( $field_id = '', $args = array() ) {
		...
		$data = 'options-page' === $type
			? wpvr_cmb_Meta_Box::get_option( $id, $field_id )
	...
	public function update_data( $new_value, $single = true ) {
		...
		if ( 'options-page' === $type )
			return wpvr_cmb_Meta_Box::update_option( $id, $field_id, $new_value, $single );
	...
	public function remove_data( $old = '' ) {
		...
		return 'options-page' === $type
			? wpvr_cmb_Meta_Box::remove_option( $id, $field_id )
	...
	public function sanitization_cb( $meta_value ) {
		...
		$clean = new wpvr_cmb_Meta_Box_Sanitize( $this, $meta_value );
		// Validation via 'wpvr_cmb_Meta_Box_Sanitize' (with fallback filter)
	...
	public function field_timezone_offset() {
		return wpvr_cmb_Meta_Box::timezone_offset( $this->field_timezone() );
	}
	...
	public function render_field() {
		...
		$wpvrClass = $this->args( 'wpvrClass' );
		$wpvrService = $this->args( 'wpvrService' );
		$wpvrType = $this->args( 'wpvrType' );
		$wpvrStyle = $this->args( 'wpvrStyle' );
		$wpvrMaxItems = $this->args( 'wpvrMaxItems' );

		$wpvr_attributes = $this->args( 'wpvr_attributes' );

		$wpvr_attributes_string = '';
		if( !is_array($wpvr_attributes) || count( $wpvr_attributes ) == 0) $wpvr_attributes_string = '' ;
		else{
			foreach( (array) $wpvr_attributes as $key => $value ) $wpvr_attributes_string .= ' '.$key.' = "'.$value.'" ';
		}
		...
		printf(
			"<tr class=\"%s\" maxItems =\"%s\" service=\"%s\" sourceType=\"%s\" style=\"%s\" %s>\n",
				$classes ,
				$wpvrMaxItems,
				$wpvrService ,
				$wpvrType ,
		...
		$this_type = new wpvr_cmb_Meta_Box_types( $this );
		$this_type->render();
	...
	public function _set_field_defaults( $args ) {
		...
		$args['options']    = 'group' == $args['type'] ? wp_parse_args( $args['options'], array(
			'add_button'    => __( 'Add Group',  WPVR_LANG  ),
			'remove_button' => __( 'Remove Group',  WPVR_LANG  ),
		) ) : $args['options'];
