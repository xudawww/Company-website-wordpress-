...
class wpvr_cmb_Meta_Box_types {
	 ...
	 */
	public function option( $opt_label, $opt_value, $selected , $opt_service = '') {
		return sprintf( "\t".'<option service="%s" value="%s" %s>%s</option>', $opt_service , $opt_value, selected( $selected, true, false ), $opt_label )."\n";
	}
	...
	public function concat_options( $args = array(), $method = 'list_input' ) {
		...
		foreach ( (array) $options as $option_key => $option ) {
			...
			$opt_service  = is_array( $option ) && array_key_exists( 'service', $option ) ? $option['service'] : '';
			...
			if ( ! empty( $args ) ) {
				...
				$this_args['service'] = $opt_service;
				if ( $is_current )
					$this_args['checked'] = 'checked';

				$_options .= $this->$method( $this_args, $i );
			} else {
				$_options .= $this->option( $opt_label, $opt_value, $is_current , $opt_service );
			}
			$i++;
		}
		return $_options;
	...
	public function list_input( $args = array(), $i ) {
		$args = $this->parse_args( $args, 'list_input', array(
			...
			'label' => '',
			'service' => $this->field->args( 'service' ),
		) );
	...
	public function render_repeatable_field() {
		...
		<p class="add-row">
			<a data-selector="<?php echo $table_id; ?>" class="add-row-button button" href="#"><?php _e( 'Add Row',  WPVR_LANG  ); ?></a>
		</p>
	...
	protected function repeat_row( $class = 'repeat-row' ) {
		...
		<tr class="<?php echo $class; ?>">
			...
			<td class="remove-row">
				<a class="button remove-row-button" href="#"><?php _e( 'Remove',  WPVR_LANG  ); ?></a>
			</td>
		</tr>
	...
	/* by pressaholic */
	public function _wpvrClass() {
		// Prevent description from printing multiple times for repeatable fields
		return $this->field->args( 'wpvrClass' );
		
	}
	public function _wpvrMasterId() {
		return $this->field->args( 'wpvrMasterId' );
	}

	public function _wpvrMasterValue() {
		return $this->field->args( 'wpvrMasterValue' );
	}


	public function _wpvrService(  ) {
		// Prevent description from printing multiple times for repeatable fields
		return $this->field->args( 'service' ) ;
		
	}
	public function _wpvrHTML(  ) {
		// Prevent description from printing multiple times for repeatable fields
		return $this->field->args( 'html' ) ;
	...
	public function select_timezone() {
		$this->field->args['default'] = $this->field->args( 'default' )
			...
			: wpvr_cmb_Meta_Box::timezone_string();
	...
	public function taxonomy_radio() {
		...
		if ( ! $terms ) {
			$options .= '<li><label>'. __( 'No terms',  WPVR_LANG  ) .'</label></li>';
		} else {
			foreach ( (array) $terms as $term ) {
				$args = array(
					'value' => $term->slug,
	...
	public function file_list() {
		...
		$this->input( array(
			...
			'value'  => __( 'Add or Upload File',  WPVR_LANG  ),
			'name'  => '', 'id'  => '',
		) );
		...
		if ( $meta_value && is_array( $meta_value ) ) {
			...
			foreach ( (array) $meta_value as $id => $fullurl ) {
				...
				if ( $this->is_valid_img_ext( $fullurl ) ) {
					...
					'<li class="img_status">',
						...
						'<p class="cmb_remove_wrapper"><a href="#" class="cmb_remove_file_button">'. __( 'Remove Image',  WPVR_LANG  ) .'</a></p>
						'. $id_input .'
					</li>';
					...
					echo
					'<li>',
						__( 'File:',  WPVR_LANG  ), ' <strong>', $title, '</strong>&nbsp;&nbsp;&nbsp; (<a href="', $fullurl, '" target="_blank" rel="external">'. __( 'Download',  WPVR_LANG  ) .'</a> / <a href="#" class="cmb_remove_file_button">'. __( 'Remove',  WPVR_LANG  ) .'</a>)
						'. $id_input .'
					</li>';
				}
			}
	...
	public function file() {
		...
		'<input class="cmb_upload_button button" type="button" value="'. __( 'Add or Upload File',  WPVR_LANG  ) .'" />',
		$this->_desc( true );
		...
		$this->field = new wpvr_cmb_Meta_Box_field( $args, $this->field->group );
		...
		if ( $meta_value && ! $_id_value ) {
			$_id_value = wpvr_cmb_Meta_Box::image_id_from_url( esc_url_raw( $meta_value ) );
		}
		...
		'<div id="', $this->_id( '_status' ) ,'" class="cmb_media_status">';
			if ( ! empty( $meta_value ) ) {
				...
				if ( $this->is_valid_img_ext( $meta_value ) ) {
					...
					echo '<p class="cmb_remove_wrapper"><a href="#" class="cmb_remove_file_button" rel="', $cached_id, '">'. __( 'Remove Image',  WPVR_LANG  ) .'</a></p>';
					echo '</div>';
				} else {
					...
					echo __( 'File:',  WPVR_LANG  ), ' <strong>', $title, '</strong>&nbsp;&nbsp;&nbsp; (<a href="', $meta_value, '" target="_blank" rel="external">'. __( 'Download',  WPVR_LANG  ) .'</a> / <a href="#" class="cmb_remove_file_button" rel="', $cached_id, '">'. __( 'Remove',  WPVR_LANG  ) .'</a>)';
				}
			}
		echo '</div>';
	}
	...
	public function oembed() {
		...
		'<div id="',$this->_id( '_status' ) ,'" class="cmb_media_status ui-helper-clearfix embed_wrap">';
			...
			if ( $meta_value = $this->field->escaped_value() ) {
				echo wpvr_cmb_Meta_Box_ajax::get_oembed( $meta_value, $this->field->object_id, array(
