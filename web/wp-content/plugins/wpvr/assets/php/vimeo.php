 ...
 *
 */
	if( class_exists( 'Vimeo' , FALSE ) ) return false;
class Vimeo
{
    ...
    public function request($url, $params = array(), $method = 'GET', $json_body = true)
    {
		...
		$response['body'] = wpvr_json_decode($response['body'], FALSE);
 ...
 */
	if( !class_exists('VimeoUploadException') ) {
