<?php
	 ...
	 */
	if ( class_exists( 'Dailymotion', false ) ) {
		return false;
	}
	...
	class Dailymotion {
		 ...
		 * @var string
		 */
		public $cookieDomain = null; // 1 year
		/**
		...
		public function call( $resource, $args = array() ) {
			...
			$payload    = wpvr_json_encode( array(
				'call' => $resource,
				'args' => $args,
			) );
			$statusCode = null;
			...
			$result = wpvr_object_to_array( $result );
			if ( is_object( $result ) ) {
				$result = (array) $result;
			}
			...
			if ( $this->debug ) {
				...
				if ( php_sapi_name() === 'cli' ) {
					...
					echo ( is_array( $payload ) )
						? wpvr_json_encode( $payload, JSON_PRETTY_PRINT )
						: ( ( is_null( $json = wpvr_json_decode( $payload ) ) )
							? $payload
							:
							wpvr_json_encode( $json, JSON_PRETTY_PRINT )
						);
					...
					echo ">>> [{$url}] >>>" . PHP_EOL;
					$message = print_r( is_null( $json = wpvr_json_decode( $payload ) ) ? $payload : $json, true );
		...
		protected function debugResponse( $url, $payload, array $headers ) {
			if ( $this->debug ) {
				...
				if ( php_sapi_name() === 'cli' ) {
					...
					echo PHP_EOL;
					echo ( $json = wpvr_json_decode( $payload, true ) ) == null ? $payload : wpvr_json_encode( $json, JSON_PRETTY_PRINT );
					echo PHP_EOL;
				} // debug for the rest
				else {
					echo "<<< [{$url}] <<<" . PHP_EOL;
					print_r( ( $json = wpvr_json_decode( $payload ) ) == null ? $payload : $json );
					echo PHP_EOL;
				}
			}
		}
