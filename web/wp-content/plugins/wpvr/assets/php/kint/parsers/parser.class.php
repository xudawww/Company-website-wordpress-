...
abstract class kintParser extends kintVariableData
{
	...
	public final static function factory( & $variable, $name = null )
	{
		...
		if ( $varType === 'object' ) {
			foreach ( self::$_objectParsers as $parserClass ) {
				...
				if ( ( $alternativeTabs = $object->parse( $variable ) ) !== false ) {
					...
					foreach ( (array) $alternativeTabs as $name => $values ) {
						$alternative       = kintParser::factory( $values );
						$alternative->type = $name;
						if ( wpvrKint::enabled() === wpvrKint::MODE_RICH ) {
		...
		if ( wpvrKint::enabled() === wpvrKint::MODE_RICH && !self::$_skipAlternatives ) {
	...
	{
		return wpvrKint::$maxLevels != 0 && self::$_level >= wpvrKint::$maxLevels;
	}

	private static function _isArrayTabular( array $variable )
	{
		if ( wpvrKint::enabled() !== wpvrKint::MODE_RICH ) return false;

		$arrayKeys   = array();
		$keys        = null;
		$closeEnough = false;
		foreach ( (array) $variable as $row ) {
	...
	public static function escape( $value, $encoding = null )
	{
		...
		if ( wpvrKint::enabled() === wpvrKint::MODE_CLI ) {
			$value = str_replace( "\x1b", "\\x1b", $value );
		}

		if ( wpvrKint::enabled() === wpvrKint::MODE_CLI || wpvrKint::enabled() === wpvrKint::MODE_WHITESPACE ) return $value;
	...
	private static function _parse_array( &$variable, kintVariableData $variableData )
	{
		...
		if ( $variableData->size > 1 && ( $arrayKeys = self::_isArrayTabular( $variable ) ) !== false ) {
			...
			foreach ( (array) $variable as $rowIndex => $koko ) {
				...
				foreach ( (array) $arrayKeys as $key ) {
					if ( $firstRow ) {
			...
			foreach ( (array) $variable as $key => $koko ) {
	...
	private static function _parse_object( &$variable, kintVariableData $variableData )
	{
		...
		if ( wpvrKint::enabled() === wpvrKint::MODE_RICH && wpvrKint::$fileLinkFormat && $reflector->isUserDefined() ) {
			$url = wpvrKint::getIdeLink( $reflector->getFileName(), $reflector->getStartLine() );
		...
		foreach ( (array) $castedArray as $key => $value ) {
		...
		foreach ( (array) $reflector->getProperties() as $property ) {
			$name = $property->name;
			...
			if ( isset( $meta['uri'] ) ) {
				...
				if ( function_exists( 'stream_is_local' ) ) {
					...
					if ( stream_is_local( $file ) ) {
						$file = wpvrKint::shortenPath( $file );
					}
				}
	...
	private static function _parse_string( &$variable, kintVariableData $variableData )
	{
		...
		if ( !self::$_placeFullStringInValue ) {
			...
			if ( wpvrKint::$maxStrLength && $variableData->size > wpvrKint::$maxStrLength ) {

				// encode and truncate
				$variableData->value         = '"'
					. self::escape( self::_substr( $strippedString, 0, wpvrKint::$maxStrLength, $encoding ), $encoding )
					. '&hellip;"';
