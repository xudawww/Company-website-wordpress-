...
class Kint_Parsers_ClassMethods extends kintParser
{
	...
	protected function _parse( &$variable )
	{
		...
		if ( !isset( self::$cache[ $className ] ) ) {
			...
			// Class methods
			foreach ( (array) $reflection->getMethods() as $method ) {
				$params = array();
				...
				// Method parameters
				foreach ( (array) $method->getParameters() as $param ) {
					$paramString = '';
					...
					if ( $param->isDefaultValueAvailable() ) {
						if ( is_array( $param->getDefaultValue() ) ) {
							$arrayValues = array();
							foreach ( (array) $param->getDefaultValue() as $key => $value ) {
				...
				$fileName = wpvrKint::shortenPath( $method->getFileName() ) . ':' . $method->getStartLine();
