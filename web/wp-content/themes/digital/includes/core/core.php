...
function digital_core_style() {
	...
	wp_enqueue_style( 'digital-responsive', $directory_uri . '/includes/core/bootstrap/css/responsive.css', array( 'bootstrap', 'bootstrap-responsive' ) );
	wp_enqueue_style( 'core_style', get_template_directory_uri() . '/includes/core/css/core.css' );
}

// Add core page to the menu.
function digital_add_core() {
	$page = add_theme_page(
		'MORE Themes',
		'MORE Themes',
		'administrator',
		'digital-themes',
		'digital_display_core'
	);

	add_action( 'admin_print_styles-' . $page, 'digital_core_style' );
}

add_action( 'admin_menu', 'digital_add_core' );

// Define markup for the core page.
function digital_display_core() {
			...
			<div id="core_container">
				<div class="row-fluid">
					<div id="core_header" class="span12">
						...
						<h3><?php _e( 'You Should Try Our Other Themes Also', 'digital' ); ?></h3>
					</div>
				</div>
				...
				<div id="core_themes" class="row-fluid">
					...
					$themes = digital_get_themes( $request );
					...
					foreach ( $themes->themes as $theme ) {
						if( $active_theme != $theme->name ) {
							...
							$theme_details = digital_get_themes( $request );
							?>
...
// Get all digital themes by using API.
function digital_get_themes( $request ) {

	// Generate a cache key that would hold the response for this request:
	$key = 'digital_' . md5( serialize( $request ) );
