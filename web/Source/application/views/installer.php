...
<title>codeigniter Duplicator</title>
<style>
...
<div id="content">
	<div id="content-inner">
    	...
    	<table class="dup-step1-inputs">
    	    ...
    	    <tr><td>Host</td><td><input type="text" name="dbhost" id="dbhost" class="regcom"  placeholder="localhost" /></td></tr>
			<tr><td>Name</td><td><input type="text" name="dbname" id="dbname" class="regcom"   placeholder="new or existing database name"  /></td></tr>
			<tr><td>User</td><td><input type="text" name="dbuser" id="dbuser" class="regcom"  placeholder="valid database username" /></td></tr>
		...
		<input type="button" name="add_floor"  value="Save Changes " id="db-connect" class="button button-primary"/>
</div>
		</form>
...
$(document).ready(function(){
	...
	var stitle = $('#dbhost').val();	
     suser = $('#dbname').val();
     shost = $('#dbuser').val();
    	
     	 if(!stitle){
			
			    $( "#dbhost" ).addClass('error-admin');
				$("#dbhost").focus();
				return false;
		   } 
		   if(!suser){
			
			    $( "#dbname" ).addClass('error-admin');
				$("#dbname").focus();
				return false;
		   } 
		   if(!shost){
			
			    $( "#dbuser" ).addClass('error-admin');
				$("#dbuser").focus();
				return false;
		   } 
		$('.loader').show();  
	 var value =$("#db_regss").serialize() ;
...
success:function(res){
		...
		$.ajax({
url:'<?php echo base_url();?>installer/select_country',
type:'post',
data:value,
success:function(res){
	
	$("#country-result").html(res);
	}
});
