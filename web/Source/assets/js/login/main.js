jQuery(document).ready(function($){
	var $form_modal = $('.cd-user-modal'),
		...
		$form_forgot_password1 = $form_modal.find('#cd-reset-details'),
...
//$('body').css("overflow","scroll");
	...
	$main_nav2.on('click', function(event){
		...
		if( $(event.target).is($main_nav2) ) {
			...
			$form_modal.addClass('is-visible');	
			//show the selected form
			( $(event.target).is('#c-details') ) ?signup_selected() : forgot_password_selected1() ;
		}
	...
	$main_nav1.on('click', function(event){
		...
		if( $(event.target).is($main_nav1) ) {
			...
			$form_modal.addClass('is-visible');	
			//show the selected form
			( $(event.target).is('#c-password') ) ?signup_selected() : forgot_password_selected() ;
		}
	...
	function forgot_password_selected1(){
		$form_login.removeClass('is-selected');
		$form_signup.removeClass('is-selected');
